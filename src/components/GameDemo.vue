<template>
  <div>
    <div v-for="pad in pads">
      <b>[{{ pad.gamepad.index }}] {{ pad.gamepad.id }}</b><br/>
      <svg width="512px" height="512px" viewBox="0 0 512 512">
        <circle cx="256" cy="256" r="200" style="fill:gray;"></circle>
        <circle :cx="256" :cy="256" r="25" style="fill:transparent;stroke:green;"></circle>
        <circle :cx="256+pad.axes[2]*200" :cy="256+pad.axes[3]*200" r="30" style="fill:transparent;stroke:blue;"></circle>
        <circle :cx="256+pad.axes[0]*200" :cy="256+pad.axes[1]*200" r="20" style="fill:black;"></circle>
      </svg>
    </div>
...
class GamepadWrapper {
  ...
  public refresh() {
    let gp = (navigator.getGamepads()[this.gamepad.index] ?? this.gamepad) as ExtraGamepad
    for (let i = 0; i < this.axes.length; i++)
      this.axes[i] = gp.axes[i];
